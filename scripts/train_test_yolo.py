...
def train_model(
    model: ultralytics.models.yolo.model.YOLO, training_args: dict, tuning_args: dict, run_num: int
) -> bool:
...
def test_model(model: ultralytics.models.yolo.model.YOLO, test_args: dict, name: str, run_num: int, save=False) -> bool:
    try:
        t0 = time()
...
def train_test_model(
    ...
    for run_num, pair in list(enumerate(grid))[skip:]:
        ...
        if train_model(model, training_args, tuning_args, run_num):
            test_model(model, test_args, training_args["name"], run_num, save=save)
